...
/**
 * CabecalhoNavegacao - Cabeçalho com navegação principal.
 *
 * Cinco variantes: simples (brand + nav-pills), centralizado (somente pills),
 * autenticacao (brand + nav + botões), escuro (fundo dark + campo de busca),
 * avatar (dropdown do usuário com busca).
 *
 * API:
 * - variante: tipo da variante
 * - itens: LinkNav[] — links de navegação
...
const props = withDefaults(
  defineProps<{
    variante: 'simples' | 'centralizado' | 'autenticacao' | 'escuro' | 'avatar';
    itens: LinkNav[];
    marca?: string;
    nomeUsuario?: string;
    avatarSrc?: string;
...
<template>
  <header v-if="variante === 'escuro'" :class="classesHeader">
    <div class="container">
      <div
